...
class AddDropCourses extends Component {
  ...
  processResults(sid,stats,term,year) {
    this.setState({courses:stats, sid:sid,term:term,year:year[0]}, () => {
    ...
    return (
      <div>
       <table>
        ...
        <GSRequest callbackParent={this.processResults} dept={this.props.location.state.department} sid={this.props.location.state.sid} />
  ...
  onInputChange(fld,val) {
    ...
    this.setState(x, () => {
      if ((this.state.term != "") && (this.state.year != "")) {
        ...
        var gradurl = "http://localhost:5000/paws/view_schedule/"+this.state.term+"/"+this.state.year+"/"+this.props.sid;
          console.log(gradurl);
         $.ajax({
          url: gradurl,
          dataType: 'json',
...
class GSResult extends React.Component {
   ...
   for(let i=0;i<values.selectedList.length;i++)
    ...
    console.log(payload);
      axios.post(apiBaseUrl+'/drop', payload).then(function (response) {
